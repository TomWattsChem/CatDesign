                    ...
                    for i in range(0, natoms):
                        for partner in self.CONNECTIVITY[i]:
                            ...
                            for otherpartner in self.CONNECTIVITY[nextatom]:
                                ...
                                othernextatom = int(otherinfo[0])-1
                                if othernextatom==i: nope=nope+1
                            if nope==0: self.CONNECTIVITY[nextatom].append(str(i+1)) # add the connectivity to the nextatom
                else:
...
class getoutData:
    def __init__(self, file):
        ...
        def accepted_file(file):
            filesplit = file.split(".")
            if len(filesplit) > 1: # there is an extension
                for suffix in ["out", "pdb"]: # authorized suffix
                    if filesplit[len(filesplit)-1] == suffix: return True
            return False
...
class calcSterimol:
   def __init__(self, file, radii, atomA, atomB,verbose):
      ...
      if len(file.split(".pdb"))>1 or len(file.split(".pdb"))>1: fileData = getinData(file)
      if len(file.split(".out"))>1 or len(file.split(".log"))>1: fileData = getoutData(file)

      # initialize the array of atomic vdw radii
      molcart = fileData.CARTESIANS; atomtype = fileData.ATOMTYPES; natoms = len(molcart); vdw_radii = [] ; con=fileData.CONNECTIVITY
      #print(con)  
      nodes=[]
      edges=[]
     ...
     #G.add_nodes_from(nodes)
      ...
      #print(list(G.nodes))
      G.remove_edge(str(atomA)+"__1.0", str(atomB)+"__1.0")
      #print(G.number_of_edges())
      sub=list(nx.connected_components(G))
      #print(sub)
      for i in sub:
         if str(atomB)+"__1.0" in i:
            subatoms=i
      #print (subatoms)
      new_set ={(int(x.replace('__1.0', ''))) for x in subatoms}
      print(new_set)  
      n_list= np.array(list(new_set)) -1
      print(n_list)
...
# subtract one since the array starts from zero not one
      ...
      for i in atomlist:
         ...
         #print("L",adjdist+vdw_radii[i]+0.40,i)
         #print("B5",oppdist+vdw_radii[i],i)
   ...
   # A bit weird, but seems like original sterimol adds on the difference between the bond length and vdw radius of atom B. For a C-H bond this is 1.50 - 1.10 = 0.40 Angstrom)
      ...
      #print((adjlist))
      #print((atomlist))
      Lall=np.asarray(adjlist)
      #print(Lall.shape)
      conL=[]
      for i in n_list:
          #print(i)
          lid=atomlist.index(i)
          conL.append(Lall[lid])
      #print(conL)
      #print(max(conL))
      #print(opplist)
      ...
      for i in n_list: zcarts.append(np.subtract(molcart[i],molcart[atomA]))
      #print(len(zcarts))
      #print(zcarts)
      #print(atomlist)
      #print(n_list)
      ...
      for t in n_list: fragrad.append(vdw_radii[t])
      singledist=[]
         ...
         for y in range(len(twodcarts)):
            if x!=y:
               ...
               if np.cross(nvect,xradv)<0.000000001 and theta!=np.pi/2:
                  ...
                  if len(satpoint)==len(n_list)-2:vlist.append(np.linalg.norm(nvect));alist.append([x,y]);#print x,y

                  satpoint=[]
                  for z in range(len(twodcarts)):
                     pvdist=twod_dist(twodcarts[z],mewx,mewy)
                     if z!=x and z!=y and pvdist>(fragrad[z]-0.0001):satpoint.append(pvdist)
                  if len(satpoint)==len(n_list)-2:vlist.append(np.linalg.norm(mvect));alist.append([x,y])
      #print(len(fragrad))
      #print(len(vlist))
      #print(len(alist))
      #print(len(atomlist))
      ...
      #print(max(fragrad),min(vlist),linearcheck(twodcarts)) 
